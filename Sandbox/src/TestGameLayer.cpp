...
TestGameLayer::TestGameLayer()
	...
	m_CameraController(1280.0f / 720.0f, 2.0f, true),
	m_StartingRabbitPosition(-2.4f, -0.92f, 0.0f),
...
{
	// Load textures
	m_RabbitTexture = Engine::Texture2D::Create("assets/textures/rabbit_sprite.png");
...
void TestGameLayer::AddNewTree()
{
	...
	{
	case 0:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100)/ 50.0f), -0.55f, 0.0f }, { 0.8f, 1.4f }, m_TreeTextures[newTreeIndex] });
		break;
	case 1:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -0.5f, 0.0f }, { 2.6f, 2.2f }, m_TreeTextures[newTreeIndex] });
		break;
	case 2:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -0.6f, 0.0f }, { 0.8f, 1.4f }, m_TreeTextures[newTreeIndex] });
		break;
	case 3:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -0.55f, 0.0f }, { 0.8f, 1.4f }, m_TreeTextures[newTreeIndex] });
		break;
	case 4:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -1.1f, 0.0f }, { 0.5f, 0.4f }, m_TreeTextures[newTreeIndex] });
		break;
	case 5:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -1.1f, 0.0f }, { 0.5f, 0.4f }, m_TreeTextures[newTreeIndex] });
		break;
	case 6:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -0.4f, 0.0f }, { 0.8f, 2.0f }, m_TreeTextures[newTreeIndex] });
		break;
	case 7:
		m_Trees.push_back({ { 8.0f - ((float)(rand() % 100) / 50.0f), -0.4f, 0.0f }, { 2.6f, 2.2f }, m_TreeTextures[newTreeIndex] });
		break;
	}
}
...
void TestGameLayer::OnUpdate(Engine::Timestep deltaTime)
{
	...
	// Draw background
	Engine::Renderer2D::DrawTexturedQuad({ 0.0f, 0.0f, 0.0f }, { 16.0f * 0.7f, 9.0f * 0.7f }, m_BackgroundTexture);
	...
	// Draw the rabbit
	Engine::Renderer2D::DrawTexturedQuad(m_RabbitPosition, { 1.0f, 1.0f }, m_RabbitTexture, m_RabbitAngle);
